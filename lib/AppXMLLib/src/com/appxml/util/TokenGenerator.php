...
class TokenGenerator {
	...
	public function verify($token, $callable = null):bool {
		$els = explode(".", $token);
        if (count($els) != 2) return false;
        if (self::hashSalt($els[0], $els[1]) != $els[2]) return false;
        $payload = json_decode($els[1], true);
	...
	public static function generate(array $payload, $salt = ""):string {
		$uuid = UUID::generate("token");
	...
	private static function hashSalt($data, $salt) {
		...
		$salt = substr($intermediateSalt, 0, 6) . $salt;
